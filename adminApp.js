function setAdminDetails(mobileNumber) {
  ...
  fetch(
    `https://banking-9ki8.onrender.com/admin/getByMobile?mobileNumber=${mobileNumber}`,
    {
      method: "GET",
      credentials: "include",
       headers: {
...
function checkUser(accountNumber) {
 ...
 const token=localStorage.getItem("jwtTokenAdmin");
  fetch(
    `https://banking-9ki8.onrender.com/admin/getByAccount?accountNumber=${accountNumber}`,
    {
      method: "GET",
      credentials: "include",
       headers: {
...
function depositBalanceToUser(accountNumber, amount) {
 const token=localStorage.getItem("jwtTokenAdmin");
  fetch(
    `https://banking-9ki8.onrender.com/admin/deposit?accountNumber=${accountNumber}&ammount=${amount}`,
    {
      method: "PATCH",
      credentials: "include",
       headers: {
...
function withdrawBalanceFromUser(accountNumber, amount, pin) {
   const token=localStorage.getItem("jwtTokenAdmin");
  fetch(
    `https://banking-9ki8.onrender.com/admin/withdraw?accountNumber=${accountNumber}&ammount=${amount}&pin=${pin}`,
    {
      method: "PATCH",
      credentials: "include",
       headers: {
